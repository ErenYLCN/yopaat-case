...
const assetValues = ref(
  data.value?.reduce<UserAssets>((dict, obj) => {
    dict[obj.symbol] = 0
    return dict
  }, {}),
)
...
watch(debouncedSearchInput, () => {
  if (!debouncedSearchInput.value.length) {
    ...
    filteredData.value = data.value.filter(asset =>
      asset.symbol.includes(debouncedSearchInput.value.toUpperCase()),
    )
  }
})
        ...
        >
          ...
          <div v-if="assetValues" flex flex-col items-center gap-2 sm:flex-row>
            ...
            <div v-if="!userAssets[asset.symbol]" w-48>
              <button w-full text-sm btn @click="addAsset(asset)">
                Add
              </button>
            </div>
            <div
            ...
            >
              ...
              <button w-full text-sm btn @click="removeAsset(asset)">
                Remove
              </button>
            </div>
          </div>
